...
function isUrgentImpact(item = {}) {
  ...
  const urgentTitle = /авар|чс\b|пожар|взрыв|отключ|обесточ|перекры|эвакуац|шторм|опасност|огранич[^.!?]{0,80}(?:водоснаб|электроснаб|газоснаб|движен)/i;
  const urgentLead = /(?:авар|чс\b|пожар|взрыв|эвакуац|шторм|опасност)|(?:будет|будут|временно|планируется|произошл[оа]?|введен[оы]?|ограничен[оаы]?|прекращен[оаы]?|отключен[оаы]?)[^.!?]{0,120}(?:водоснаб|электроснаб|газоснаб|подач[аи]\s+(?:вод|электр|газ)|движен)|(?:отключат|обесточат|перекроют)[^.!?]{0,120}/i;
  return urgentTitle.test(title) || urgentLead.test(lead);
}
