...
async function fetchText(url) {
  const r = await fetch(url, {
    headers:{"User-Agent":"MakhachkalaLIVE/2.11.0 (+telegram @mkala_live05)"},
    redirect:"follow",
...
function extractTelegramPosts(html, channel, sourceName) {
  ...
  if (!ch) return [];
  const escaped = ch.replace(/[.*+?^${}()|[\\]\\\\]/g,"\\\\function filterMchsLocal(items) {
  return items.filter(x => !isPolitical(`${x.title} ${x.description}`) && isMakhachkalaText(`${x.title} ${x.description}`));
}
");
  const hits=[...String(html).matchAll(new RegExp(`data-post=["\']${escaped}\\/(\\d+)["\']`, "gi"))];
  const out=[];
  for(let i=0;i<hits.length;i++){
    ...
    const description=stripHtml(textHtml.replace(/<br\\s*\\/?\\s*>/gi,". "));
    const pubDate=datetime ? new Date(datetime) : null;
    const id=hits[i][1];
    const title=deriveTelegramTitle(description);
...
module.exports = {
  ...
  fetchTelegramPublicChannel,fetchMakhachkalaAdminTelegram,fetchMintransTelegram,
  extractTelegramPosts,deriveTelegramTitle,
