const {decodeEntities, stripHtml, isMakhachkalaText, isPolitical, parseRuDateText, uniqueBy} = require("./common.cjs");
...
async function fetchText(url) {
  const r = await fetch(url, {
    headers:{"User-Agent":"MakhachkalaLIVE/2.6 (+telegram @mkala_live05)"},
    redirect:"follow",
...
function extractDescriptionFromArticle(html) {
  ...
  const paragraphs = [...html.matchAll(/<p[^>]*>([\s\S]*?)<\/p>/gi)]
    .map(m => stripHtml(m[1]))
    .filter(Boolean)
    .filter(x => !/^МАХАЧКАЛА[,，]/i.test(x));
  return paragraphs.find(x => x.length >= 80) || "";
}
