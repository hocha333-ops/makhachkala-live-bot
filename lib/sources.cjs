...
async function fetchText(url) {
  const r = await fetch(url, {
    headers:{"User-Agent":"MakhachkalaLIVE/2.7.1 (+telegram @mkala_live05)"},
    redirect:"follow",
...
async function fetchMchsFeed(path, sourceName) {
  ...
  return blocks.map(block => ({
    title:pickTag(block,"title"), link:pickTag(block,"link"), description:pickTag(block,"description"),
    pubDate:parseRssDate(pickTag(block,"pubDate")), source:sourceName
...
function extractDescriptionFromArticle(html) {
  ...
  for (const p of patterns) {
    const m = html.match(p);
    if (m) {
      const value = cleanSourceDateline(stripHtml(m[1]));
  ...
  const paragraphs = [...html.matchAll(/<p[^>]*>([\s\S]*?)<\/p>/gi)]
    .map(m => cleanSourceDateline(stripHtml(m[1])))
    .filter(x => x.length >= 40);

  for (const p of paragraphs) {
    const normalized = p.toLowerCase().replace(/\s+/g," ").trim();
    const duplicate = parts.some(x => {
      const n = x.toLowerCase().replace(/\s+/g," ").trim();
      return n === normalized || n.includes(normalized) || normalized.includes(n);
    });
    if (!duplicate) parts.push(p);
...
module.exports = {
  ...
  extractRiaArticleUrls,extractDateFromArticle,extractTitleFromArticle,extractDescriptionFromArticle,
