const {fetchMchsFeed,fetchRiaMakhachkala,filterMchsLocal,fetchMakhachkalaAdminTelegram,fetchMintransTelegram,fetchMinobrnaukiTelegram} = require("../lib/sources.cjs");
module.exports = async function handler(req,res){
  if(req.method!=="GET") return res.status(405).json({ok:false,error:"GET only"});
  res.setHeader("Cache-Control","s-maxage=300, stale-while-revalidate=600");
  const started=Date.now();
  const [ria,cityAdmin,mintrans,minobr,mNews,mOps] = await Promise.all([
    ...
    fetchMinobrnaukiTelegram().catch(e=>({error:e.message})),
  ...
  res.status(allOk?200:207).json({ok:allOk,version:"2.11.1",elapsedMs:Date.now()-started,sources});
};
